...
class SubRuleSet1:
    def __init__(self, input_vars, output_vars, cached_vars):
        ...
        self.temp_cached_vars = cached_vars
        self.paras = []
        self.conditions = []
        self.actions = []
        self.state = None
        ...
        def condition0(self: SubRuleSet1):
            return F((self.state[:,self.index_0]) * (self.p_0 + 1.0 / 1) + self.p_1)
        ...
        def condition2(self: SubRuleSet1):
            return self.state[:,self.index_3] * (F((self.state[:,self.index_2]) * (self.p_4 + 1.0 / 100) + self.p_5))
        ...
        def condition4(self: SubRuleSet1):
            return self.state[:,self.index_6] * (F((self.state[:,self.index_5]) * (self.p_6 + 1.0 / 100) + self.p_7))
        ...
        def condition6(self: SubRuleSet1):
            return self.state[:,self.index_9] * (F((self.state[:,self.index_8]) * (self.p_8 + 1.0 / 100) + self.p_9))
    ...
    def cal_state(self):
        self.temp_cached_vars = self.cached_vars
        state = []

        state.append(((20) - self.temp_cached_vars["side_distance"]))
        ...
        state.append(1.0 if (self.temp_cached_vars["combatStage"] == 1) else 0.0)
        state.append((1000 - self.temp_cached_vars["distance"]))
